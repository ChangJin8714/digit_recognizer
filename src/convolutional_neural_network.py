...
def shared_test(data):
    ''' Shared test data for Theano library. It places the data onto a GPU
    if available'''
...
class Network(object):
    ...
    # Predict labels for the test data. The network output is assumed to be 
    # the index of neuron that has the highest activation
    def predict(self, test_data):
        data_length = len(test_data)
        test_data = shared_test(test_data)
        index = Tensor.lscalar() # mini-batch index
        test_mb_predictions = theano.function(
            [index], self.layers[-1].y_out,
            givens={
                self.x : test_data[index*self.mini_batch_size:(index+1)*self.mini_batch_size]
            })

        test_labels = []
        test_batches = data_length/self.mini_batch_size
        print 'First:',test_mb_predictions(0)[0]
        for num in xrange(test_batches):
            test_labels.extend(test_mb_predictions(num))
        return test_labels

    # Predict labels for the test data. The network output is assumed to be 
    # the index of neuron that has the highest activation
    def predict2(self, test_data, mini_batch_size):
        test_data = shared_test(test_data)
        index = Tensor.lscalar() # mini-batch index
        test_mb_predictions = theano.function(
            [index], self.layers[-1].y_out,
            givens={
                self.x : test_data[index]
            })
        print 'HERE:',test_mb_predictions(0)
        test_labels = []
        for num in xrange(size(test_data)):
            test_labels.append(test_mb_predictions(num))
        return test_labels
